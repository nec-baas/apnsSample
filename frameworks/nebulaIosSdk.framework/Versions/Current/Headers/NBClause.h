 ...
 *  @param key    演算対象のフィールド
 *  @param value  正規表現
 *  @param options オプション　設定値はNBRegexCaseOptionsを参照。複数条件は|で和をとって設定可能である。
 *
 *  @return 条件を設定したNBClauseのインスタンス
 */
+ (NBClause*)regex:(NSString*)key expression:(NSString*)value option:(NBRegexCaseOptions)options;
